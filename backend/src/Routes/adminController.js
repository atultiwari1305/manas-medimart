 ...
 const acceptRegRequest = async (req, res) => {
  ...
  try {
    ...
    const newPharmacist = new pharmacistModel({
      ...
      chatRooms: [], // ensure empty array
      IDDocument: request.IDDocument,
      pharmacyDegreeDocument: request.pharmacyDegreeDocument,
      workingLicenseDocument: request.workingLicenseDocument,
      WalletBalance: 0,
      Notifications: [],
    });

    await newPharmacist.save();
    await pharmaReqModel.findByIdAndDelete(id);

    res.status(200).json({ message: 'Pharmacist request accepted successfully' });
  } catch (error) {
    console.error('Error in acceptRegRequest:', error);
