...
const userPrescription = async (req, res) => {
  try {
    ...
    const prescriptions = user.Prescriptions || []; // fallback to empty array
  ...
  try {
    ...
    for (const med of prescription.Medicine) {
      if (prescription.Status !== "Filled") {
        const medicineData = await medModel.find({ Name: med.MedicineName });
        if (medicineData.length > 0) prescribedMedicines.push(medicineData);
      }
    }

    if (prescribedMedicines.length === 0) {
      return res.status(400).send("No medicines found");
    }
